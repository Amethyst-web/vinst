$(function () {
    ...
    function updateCart(data)
    {
        ...
        if (data.discount_numeric) {
            $(".cart-discount").closest('div.basket-row').show();
        }
    ...
    $(".cart input.qty").change(function () {
        var that = $(this);
        if (that.val() > 0) {
            var row = that.closest('tr.basket-row');
            if (that.val()) {
    ...
    $(".cart .services input:checkbox").change(function () {
        var obj = $('select[name="service_variant[' + $(this).closest('tr.basket-row').data('id') + '][' + $(this).val() + ']"]');
        if (obj.length) {
            if ($(this).is(':checked')) {
